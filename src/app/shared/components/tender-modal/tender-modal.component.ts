...
export class TenderModalComponent implements OnInit {
  ...
  private wrongClientToast = false;
  ...
  ngOnInit() {
    ...
    // console.log(JSON.parse(this.hs.getFromLocalStorage('orgList')));
    // return;
  ...
  initializeForm() {
    ...
    this.clientList = this.tenderHeaderForm.get('clientName').valueChanges.pipe(
      startWith(''),
      map(v => (v ? this.filteredClients(v) : this.allClients.slice()))
    );

    this.clientList.subscribe(_list => {
    //   console.log('client list  ... ', _list);
      if (_list != undefined) { // when name field empty list will be undefined
        if (_list.length <= 0 && this.wrongClientToast == false) {
          this.wrongClientToast = true;
 ...
 /*  removeNonExistentClient(event) {
    ...
    setTimeout(() => {
      const isValueTrue = this.allClients.filter(client => client.name === event.target.value);
  ...
  private customValidator(param): ValidatorFn {
    ...
    return (ctrl: FormControl) => {
      ...
      if (String(userInput).length < 5) {
        msg = 'Input must be at least 5 characters long';
        isValid = false;
      } else if (String(userInput).length > 30) {
        msg = 'Input must not exceed 30 characters';
        isValid = false;
      } else if (!String(userInput).match(regexp)) {
        msg = 'Input must not start or end with special characters';
        isValid = false;
  ...
  save() {
    ...
    //
    this.httpServ.addNewTender(updateData).subscribe(
      res => {
        if (res.status === 201) {
  ...
  update() {
    ...
    this.httpServ.updateTender(tenderID, updateData).subscribe(
      res => {
        if (res.status === 200) {
